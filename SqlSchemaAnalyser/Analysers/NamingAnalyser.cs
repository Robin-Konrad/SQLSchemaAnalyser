...
public class NamingAnalyser : IAnalyser
{
    public string Category { get; } = "naming";
    private AnalysisClient AC;
    private string promptVersion;

    public NamingAnalyser(AnalysisClient AC, string promptVersion = "") {
        this.AC = AC;
    ...
    public async Task<(List<Finding> Findings, LlmUsageData Usage)> AnalyseAsync(string schema) {
        ...
        PromptConfig NamingPrompt = PromptLoader.LoadPrompt("naming", this.promptVersion); // load correct PromptConfig

        var response = await this.AC.QueryApi(NamingPrompt, filteredSchema);  // make call to azure client with PromptConfig and schema (response.ReponseText response.UsageData)
